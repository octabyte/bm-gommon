...
func (s *RepositoryIntegrationSuite) SetupSuite() {
	...
	if err != nil {
		s.T().Skipf("skipping outbox integration tests, mongo container unavailable: %v", err)
	}

	host, err := container.Host(s.ctx)
	require.NoError(s.T(), err)

	port, err := container.MappedPort(s.ctx, "27017/tcp")
	require.NoError(s.T(), err)
	...
	s.client = client
	s.repo = NewRepository(client, testDBName)
}
...
func (s *RepositoryIntegrationSuite) TestEnqueue_PersistsPendingEvent() {
	...
	require.NotEmpty(s.T(), event.ID.Hex())
	require.JSONEq(s.T(), `{"booking_id":"booking-123"}`, string(event.Payload))
	require.Equal(s.T(), int32(77), event.Metadata["company_id"])
...
func (s *RepositoryIntegrationSuite) TestEnqueue_WithDestinationOption_PersistsDestination() {
	...
	err = s.collection().FindOne(s.ctx, bson.M{}).Decode(&event)
	require.NoError(s.T(), err)
...
func (s *RepositoryIntegrationSuite) TestClaimBatch_ClaimsOnlyPendingAndExpiredProcessing() {
	...
	pendingA := s.insertEvent(mongoEvent{
	...
	pendingB := s.insertEvent(mongoEvent{
		ID:          primitive.NewObjectID(),
	...
	expired := s.insertEvent(mongoEvent{
		ID:          primitive.NewObjectID(),
	...
	require.Equal(s.T(), pendingA.ID.Hex(), claimed[0].ID)
	require.Equal(s.T(), pendingB.ID.Hex(), claimed[1].ID)
	require.Equal(s.T(), expired.ID.Hex(), claimed[2].ID)
...
func (s *RepositoryIntegrationSuite) TestMarkPublished_TransitionsAndClearsLocks() {
	...
	event := s.insertEvent(mongoEvent{
		ID:          primitive.NewObjectID(),
	...
	err := s.repo.MarkPublished(s.ctx, event.ID.Hex())
	require.NoError(s.T(), err)
...
func (s *RepositoryIntegrationSuite) TestRescheduleRetry_IncrementsAttemptsAndRequeues() {
	...
	err := s.repo.RescheduleRetry(s.ctx, event.ID.Hex(), nextRetry, lastErr)
	require.NoError(s.T(), err)
...
func (s *RepositoryIntegrationSuite) TestMarkFailed_SetsFailedAndClearsLocks() {
	...
	err := s.repo.MarkFailed(s.ctx, event.ID.Hex(), "max retries reached")
	require.NoError(s.T(), err)
...
func (s *RepositoryIntegrationSuite) collection() *mongo.Collection {
	return s.client.Database(testDBName).Collection(outbox.CollectionName)
}

func (s *RepositoryIntegrationSuite) insertEvent(event mongoEvent) mongoEvent {
	if event.ID.IsZero() {
...
func (s *RepositoryIntegrationSuite) getEvent(id primitive.ObjectID) mongoEvent {
	var event mongoEvent
	err := s.collection().FindOne(s.ctx, bson.M{"_id": id}).Decode(&event)
	require.NoError(s.T(), err)
	return event
}
